...
TGEMSBSGEMSimHitData::TGEMSBSGEMSimHitData (UInt_t h)
  : fRunID(0), fEvtID(0), fSource(0)
{
  fHitData.reserve(h);
}

TGEMSBSGEMSimHitData::~TGEMSBSGEMSimHitData()
{
}

void
TGEMSBSGEMSimHitData::ClearEvent() 
{
  fHitData.clear();
};

void
TGEMSBSGEMSimHitData::InitEvent (UInt_t h)
{
  if (h <= 0)
    return;
...
void 
TGEMSBSGEMSimHitData::Print() const
{
  cout << "Run " << GetRun() << " Event " << GetEvent() << " " << GetNHit() << " hits" << endl;
}

void 
TGEMSBSGEMSimHitData::PrintHit (UInt_t k) const
{
...
void TGEMSBSGEMSimHitData::AddGEMData(TGEMSBSGEMSimHitData* gd)
{
  if( !gd ) return;
